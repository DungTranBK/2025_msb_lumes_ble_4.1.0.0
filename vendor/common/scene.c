    	...
    	foreach(i,SCENE_CNT_MAX){
    		...
    		if(p_recall->id == p->id){
    			...
    			update_control_message_params(
    						cb_par->model_idx, cb_par->adr_src, cb_par->adr_dst, SCENE_RECALL
    					);
    			bool st = (level_set_tmp.level == LEVEL_OFF)?G_OFF:G_ON;
    			send_control_dev_manual_to_mcu(cb_par->model_idx, st, true);
	#else
    			#if MD_LEVEL_EN
    			if(pv_handle_get_level != NULL) {
					#if DEV_TYPE_SEL == G_TYPE_LM_CURTAIN
					curtain_set_level(cb_par->model_idx, p->level);
					#elif DEV_TYPE_SEL == G_TYPE_LM_DIMMER
					// DIMMER CONTROL
					// ---------- CONTROL BY LEVEL ----------
					// dim_set_level(cb_par->model_idx, p->level);
					// ---------- CONTROL BY LIGHTNESS ----------
					mesh_cmd_lightness_set_t lightness_set;
					lightness_set.lightness = p->lightness;
					lightness_set.transit_t = lightness_set.tid = lightness_set.delay = 0;
					func_handle_control_message(
								(u8*)&lightness_set, sizeof(mesh_cmd_lightness_set_t), cb_par, LIGHTNESS_SET
							);

	    			// FORCE control switch binding by scene recall
	    			force_control_binding[cb_par->model_idx] = TRUE;
					#elif DEV_TYPE_SEL == G_TYPE_LM_FAN
					// FAN CONTROL
					fan_set_level(cb_par->model_idx, p->level);
					#endif
	    			// Just update if valid scene
	    			update_control_message_params(
	    						cb_par->model_idx, cb_par->adr_src, cb_par->adr_dst, SCENE_RECALL
