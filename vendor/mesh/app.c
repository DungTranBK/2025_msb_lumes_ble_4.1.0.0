...
//		DBG_APP_SEND_STR("\n MAIN: ");
//		DBG_APP_SEND_INT(temp_t/1000);
//		led_push_normal_blink_led_cmd_to_fifo(3, LED_COLOR_BLUE);
	}
#endif
}
...
void user_init()
{
	...
	handle_rx_message_callback_init(bl0906_handle_serial_rx_message);
	#endif
#endif
	#if ADC_ENABLE
	...
	auto_callback_init(send_control_dev_manual_to_mcu);

	update_dev_status_init();
	system_time_callback_init(
				update_handle_update_device_st_periodically,
				sw_loop_100ms_interval
			);
	dimming_init();
    // Led and button
	led_init();
	led_scene_init();
	led_refresh_callback_init(relay_handle_refresh);
    ...
    fact_handle_evt_change_callback_init(  \
            Switch_handleFactConfirmOrActivate, Switch_handleExitFactMode);
#endif
