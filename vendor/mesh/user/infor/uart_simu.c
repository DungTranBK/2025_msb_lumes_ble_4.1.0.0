...
#define		BIT_INTERVAL_SYS_TICK	(CLOCK_SYS_CLOCK_1S/SIM_BAUD_USE)
 ...
 * @retval None
 */
_attribute_no_inline_ void uart_info_debug_info_tx_pin_init()
{
    gpio_setup_up_down_resistor(UART_SIM_INFO_TX_PIN,PM_PIN_PULLUP_10K);
    gpio_set_func(UART_SIM_INFO_TX_PIN, AS_GPIO);
	gpio_write(UART_SIM_INFO_TX_PIN, 1);
    gpio_set_output_en(UART_SIM_INFO_TX_PIN, 1);
    gpio_set_input_en(UART_SIM_INFO_TX_PIN, 0);
}
...
static void uart_info_do_put_char(u32 pcTxReg, u8 *bit)
{
	int j;
#if SIM_BAUD_USE == SIMU_BAUD_1M
 ...
 * @retval None
 */
_attribute_ram_code_ static void uart_info_put_char(u8 byte){
	if (!tx_pin_initialed) {
		uart_info_debug_info_tx_pin_init();
		tx_pin_initialed = 1;
	}
	volatile u32 pcTxReg = (0x583+((UART_SIM_INFO_TX_PIN>>8)<<3));//register GPIO output
	u8 tmp_bit0 = read_reg8(pcTxReg) & (~(UART_SIM_INFO_TX_PIN & 0xff));
	u8 tmp_bit1 = read_reg8(pcTxReg) | (UART_SIM_INFO_TX_PIN & 0xff);
	...
	uart_info_do_put_char(pcTxReg, bit);
	if(SIM_UART_IRQ_EN)
	{
		irq_restore(r);
	}
 ...
 */
_attribute_ram_code_ void uart_info_send_bytes(u8 *p,int len)
{
    while(len--){
        uart_info_put_char(*p++);
    }
}
