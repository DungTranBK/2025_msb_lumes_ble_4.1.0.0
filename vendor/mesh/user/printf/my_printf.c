/*
 * my_printf.c
 *
 *  Created on: Nov 6, 2025
 *      Author: DungTranBK
 */


/*
 * uart_simu.c
 *
 *  Created on: Oct 2, 2024
 *      Author: DungTranBK
 */

/******************************************************************************/
/*                              INCLUDE FILES                                 */
/******************************************************************************/
#include "tl_common.h"
#include "proj_lib/ble/service/ble_ll_ota.h"
#include "my_printf.h"

#define		MBIT_INTERVAL_SYS_TICK	(CLOCK_SYS_CLOCK_1S/MY_PRINTF_BAUD_USE)

_attribute_no_retention_bss_ static int tx_pin_initialed = 0;

/**
 * @brief  DEBUG_INFO_TX_PIN initialize. Enable 1M pull-up resistor,
 *   set pin as gpio, enable gpio output, disable gpio input.
 * @param  None
 * @retval None
 */
_attribute_no_inline_ void my_printf_tx_pin_init()
{
    gpio_setup_up_down_resistor(MY_PRINTF_DEBUG_PIN,PM_PIN_PULLUP_10K);
    gpio_set_func(MY_PRINTF_DEBUG_PIN, AS_GPIO);
	gpio_write(MY_PRINTF_DEBUG_PIN, 1);
    gpio_set_output_en(MY_PRINTF_DEBUG_PIN, 1);
    gpio_set_input_en(MY_PRINTF_DEBUG_PIN, 0);
}

/* Put it into a function independently, to prevent the compiler from
 * optimizing different pins, resulting in inaccurate baud rates.
 */
_attribute_ram_code_
_attribute_no_inline_
static void my_printf_do_put_char(u32 pcTxReg, u8 *bit)
{
	int j;
#if MY_PRINTF_BAUD_USE == MY_PRINTF_BAUD_1M
	/*! Make sure the following loop instruction starts at 4-byte alignment: (which is destination address of "tjne") */
	// _ASM_NOP_;

	for(j = 0;j<10;j++)
	{
	#if CLOCK_SYS_CLOCK_HZ == 16000000
		CLOCK_DLY_8_CYC;
	#elif CLOCK_SYS_CLOCK_HZ == 32000000
		CLOCK_DLY_7_CYC;CLOCK_DLY_7_CYC;CLOCK_DLY_10_CYC;
	#elif CLOCK_SYS_CLOCK_HZ == 48000000
		CLOCK_DLY_8_CYC;CLOCK_DLY_8_CYC;CLOCK_DLY_10_CYC;
		CLOCK_DLY_8_CYC;CLOCK_DLY_6_CYC;
	#else
	#error "error CLOCK_SYS_CLOCK_HZ"
	#endif
		write_reg8(pcTxReg, bit[j]); 	   //send bit0
	}
#else
	u32 t1 = 0, t2 = 0;
	t1 = read_reg32(0x740);
	for(j = 0;j<10;j++)
	{
		t2 = t1;
		while(t1 - t2 < MBIT_INTERVAL_SYS_TICK){
			t1	= read_reg32(0x740);
		}
		write_reg8(pcTxReg,bit[j]); 	   //send bit0
	}
#endif
}


/**
 * @brief  Send a byte of serial data.
 * @param  byte: Data to send.
 * @retval None
 */
_attribute_ram_code_ static void my_printf_put_char(u8 byte){
	if (!tx_pin_initialed) {
		my_printf_tx_pin_init();
		tx_pin_initialed = 1;
	}
	volatile u32 pcTxReg = (0x583+((MY_PRINTF_DEBUG_PIN>>8)<<3));//register GPIO output
	u8 tmp_bit0 = read_reg8(pcTxReg) & (~(MY_PRINTF_DEBUG_PIN & 0xff));
	u8 tmp_bit1 = read_reg8(pcTxReg) | (MY_PRINTF_DEBUG_PIN & 0xff);


	u8 bit[10] = {0};
	bit[0] = tmp_bit0;
	bit[1] = (byte & 0x01)? tmp_bit1 : tmp_bit0;
	bit[2] = ((byte>>1) & 0x01)? tmp_bit1 : tmp_bit0;
	bit[3] = ((byte>>2) & 0x01)? tmp_bit1 : tmp_bit0;
	bit[4] = ((byte>>3) & 0x01)? tmp_bit1 : tmp_bit0;
	bit[5] = ((byte>>4) & 0x01)? tmp_bit1 : tmp_bit0;
	bit[6] = ((byte>>5) & 0x01)? tmp_bit1 : tmp_bit0;
	bit[7] = ((byte>>6) & 0x01)? tmp_bit1 : tmp_bit0;
	bit[8] = ((byte>>7) & 0x01)? tmp_bit1 : tmp_bit0;
	bit[9] = tmp_bit1;

	/*! Minimize the time for interrupts to close and ensure timely
	    response after interrupts occur. */
	u32 r = 0;
	if(MY_PRINTF_IRQ_EN)
	{
		r = irq_disable();
	}
	my_printf_do_put_char(pcTxReg, bit);
	if(MY_PRINTF_IRQ_EN)
	{
		irq_restore(r);
	}
}

/**
 * @brief  Send serial data.
 * @param  p: Data pointer to send.
 * @param  len: Data length to send.
 * @retval None
 * @note   Previously, the irq_disable and irq_restore functions were
 *   placed at the beginning and end of this function, which caused the
 *   interrupt to be turned off for a long time when sending long data,
 *   causing some interrupted code to fail to execute in time.
 */
_attribute_ram_code_ void my_printf_send_bytes(u8 *p,int len)
{
    while(len--){
        my_printf_put_char(*p++);
    }
}
